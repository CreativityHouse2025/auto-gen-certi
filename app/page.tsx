...
import { uploadCsvToBlob } from "./actions/upload-csv"
...
export default function Home() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Reset blob URL when a new file is selected
      setBlobUrl("")
    }
  }
...
const handleUpload = async (e: React.FormEvent) => {
  ...
  setLoading(true)
  setResults([])
  setUploadStatus("Uploading CSV to Blob storage...")

  try {
    // First, upload the CSV to Vercel Blob
    const formData = new FormData()
    formData.append("csv", file)

    const uploadResult = await uploadCsvToBlob(formData)

    if (!uploadResult.success || !uploadResult.url) {
      throw new Error(uploadResult.error || "Failed to upload CSV to Blob storage")
    }

    setBlobUrl(uploadResult.url)
    setUploadStatus("CSV uploaded successfully. Processing data...")

    // Then, send the Blob URL to the API using FormData
    const apiFormData = new FormData()
    apiFormData.append("blobUrl", uploadResult.url)
    apiFormData.append(
      "templates",
      JSON.stringify(TEMPLATES.filter((t) => selectedTemplates[t.id]).map((t) => t.file.split("/").pop()))
    )
    apiFormData.append("driveFolderUrl", driveFolderUrl)
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6 text-center">Certificate Generator</h1>

        <form onSubmit={handleUpload} className="space-y-8">
          {/* Template Selection */}
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-gray-700">Select Templates</h2>
              ...
              {TEMPLATES.map((template) => (
                ...
                >
                  <div className="aspect-w-1 aspect-h-1">
                    <Image
                      src={template.file || "/placeholder.svg"}
                      alt={template.name}
                      width={200}
                      height={200}
                  ...
                  <div className="mt-2 text-sm text-center font-medium text-gray-700">{template.name}</div>
                  <input
                    type="checkbox"
          ...
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-gray-700">Google Drive Folder URL</h2>
            <input
              type="text"
          ...
          <div className="space-y-4">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
              ...
              <label htmlFor="csv-upload" className="cursor-pointer flex flex-col items-center space-y-2">
                ...
                <div className="text-sm text-gray-600">
                  {file ? (
                    ...
                    <>
                      ...
                      <p className="text-xs text-gray-500 mt-1">CSV should contain columns: fullName, email</p>
                    </>
                  )}
                </div>
              </label>
            </div>
            {blobUrl && <div className="text-sm text-green-600">✓ CSV uploaded to Blob storage</div>}
          </div>
          ...
          >
            {loading ? uploadStatus || "Processing..." : "Generate Certificates"}
          </button>
        </form>
