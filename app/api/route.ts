...
const REFRESH_TOKEN =
  "1//04DkVGHciQiLWCgYIARAAGAQSNwF-L9IrNI1_wJteNVHz2r-2Q6ar3xcBc3ygYDQWjHtgd6GIkR4sfrkXrgqGCWElvK9Ss_9qrCg"
...
async function processUser(user: ProcessRequest, selectedTemplates: string[]) {
  ...
  const templates = ALL_TEMPLATES.filter((t) => selectedTemplates.includes(t.file))
  ...
  const userFolderUrl = `https://drive.google.com/drive/folders/${userFolderId}`
...
// Updated POST handler to work with Blob URLs
export async function POST(req: NextRequest) {
  try {
    const formData = await req.formData()
    const blobUrl = formData.get("blobUrl") as string
    const templatesInput = formData.get("templates") as string
    const driveFolderUrl = formData.get("driveFolderUrl") as string

    // Validate templates
    if (!templatesInput) {
      return new NextResponse(JSON.stringify({ error: "No templates selected" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      })
    }

    const selectedTemplates = JSON.parse(templatesInput) as string[]

    if (!Array.isArray(selectedTemplates)) {
      return new NextResponse(JSON.stringify({ error: "Invalid template format" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      })
    }
    ...
    if (!selectedTemplates.every((t) => VALID_TEMPLATES.includes(t))) {
      return new NextResponse(JSON.stringify({ error: "Invalid template selection" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      })
    }
    ...
    if (!blobUrl) {
      return new NextResponse(JSON.stringify({ error: "CSV Blob URL required" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      })
    }
    ...
    if (errors.length > 0) {
      return new NextResponse(JSON.stringify({ error: "Invalid CSV format", details: errors }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      })
    }
    ...
    return new NextResponse(JSON.stringify({ error: "Server error", details: errorMessage }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    })
  }
